...
> [!TIP]
> In a hurry? `gitvisualise init /path/to/your/repo` does steps 1, 2's workflow file and 6 in one go — it generates the tour,
> writes `.github/workflows/architecture.yml` if you don't already have one, and prints the README badge. It never overwrites
> an existing file, and `--dry-run` shows what it would do first. The steps below explain what each piece is for.
